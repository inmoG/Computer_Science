...
#    course = []
#
#    for page in range(last_page): # 0 ~ 3
#        page = page+1 # 0+1
#        result = requests.get(f'{URL}&page={page}')
#        soup = BeautifulSoup(result.text, 'html.parser')
...
#    title = html.find("a", {"class":"course_card_front"}).find("div", {"class": "course_title"}).string
#    link = html.find("a")["href"]
#    instructor = html.find("a", {"class":"course_card_front"}).find("div", {"class": "instructor"}).string
#    price = html.find("a", {"class":"course_card_front"}).find("div", {"class":"price"}).string
#    rating = extract_rating(html)
#
#    return {
#        "title":title,
...
#
#def extract_rating(html):
#    result = html.find("a", {"class": "course_card_front"}).find("div", {"class": "star_solid"})['style']
#    rating = float(result.lstrip("width: ").rstrip("%"))
#    return round(rating)
#
#def get_course():
#    last_page = get_last_page()
